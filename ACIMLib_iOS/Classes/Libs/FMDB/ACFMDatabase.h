...
#import "ACFMResultSet.h"
#import "ACFMDatabasePool.h"

NS_ASSUME_NONNULL_BEGIN

#if ! __has_feature(objc_arc)
    #define ACFMDBAutorelease(__v) ([__v autorelease]);
    #define ACFMDBReturnAutoreleased ACFMDBAutorelease

    #define ACFMDBRetain(__v) ([__v retain]);
    #define ACFMDBReturnRetained ACFMDBRetain

    #define ACFMDBRelease(__v) ([__v release]);

    #define ACFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define ACFMDBAutorelease(__v)
    #define ACFMDBReturnAutoreleased(__v) (__v)

    #define ACFMDBRetain(__v)
    #define ACFMDBReturnRetained(__v) (__v)

    #define ACFMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define ACFMDBDispatchQueueRelease(__v)
    #else
        #define ACFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^ACFMDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);

typedef NS_ENUM(int, ACFMDBCheckpointMode) {
...
@interface ACFMDatabase : NSObject
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) ACFMDBExecuteStatementsCallbackBlock _Nullable)block;
...
- (ACFMResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (ACFMResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (ACFMResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (ACFMResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (ACFMResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (ACFMResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;
 ...
 */
- (BOOL)checkpoint:(ACFMDBCheckpointMode)checkpointMode error:(NSError * _Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return YES on success, otherwise NO.
 */
- (BOOL)checkpoint:(ACFMDBCheckpointMode)checkpointMode name:(NSString * _Nullable)name error:(NSError * _Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return YES on success, otherwise NO.
 */
- (BOOL)checkpoint:(ACFMDBCheckpointMode)checkpointMode name:(NSString * _Nullable)name logFrameCount:(int * _Nullable)logFrameCount checkpointCount:(int * _Nullable)checkpointCount error:(NSError * _Nullable *)error;
...
@interface ACFMStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
