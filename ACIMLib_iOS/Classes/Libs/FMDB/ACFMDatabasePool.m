...
- (void)pushDatabaseBackInPool:(ACFMDatabase*)db;
- (ACFMDatabase*)db;

@end


@implementation ACFMDatabasePool
...
+ (instancetype)databasePoolWithURL:(NSURL *)url {
    return ACFMDBReturnAutoreleased([[self alloc] initWithPath:url.path]);
}

+ (instancetype)databasePoolWithPath:(NSString *)aPath flags:(int)openFlags {
    return ACFMDBReturnAutoreleased([[self alloc] initWithPath:aPath flags:openFlags]);
}

+ (instancetype)databasePoolWithURL:(NSURL *)url flags:(int)openFlags {
    return ACFMDBReturnAutoreleased([[self alloc] initWithPath:url.path flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags vfs:(NSString *)vfsName {
    ...
    if (self != nil) {
        ...
        _databaseInPool     = ACFMDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = ACFMDBReturnRetained([NSMutableArray array]);
...
- (void)dealloc {
    ...
    _delegate = 0x00;
    ACFMDBRelease(_path);
    ACFMDBRelease(_databaseInPool);
    ACFMDBRelease(_databaseOutPool);
    ACFMDBRelease(_vfsName);
    
    if (_lockQueue) {
        ACFMDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
...
- (void)inDatabase:(__attribute__((noescape)) void (^)(ACFMDatabase *db))block {
    
    ACFMDatabase *db = [self db];
...
- (void)beginTransaction:(FMDBTransaction)transaction withBlock:(void (^)(ACFMDatabase *db, BOOL *rollback))block {
...
- (void)inTransaction:(__attribute__((noescape)) void (^)(ACFMDatabase *db, BOOL *rollback))block {
    [self beginTransaction:FMDBTransactionExclusive withBlock:block];
}

- (void)inDeferredTransaction:(__attribute__((noescape)) void (^)(ACFMDatabase *db, BOOL *rollback))block {
    [self beginTransaction:FMDBTransactionDeferred withBlock:block];
}

- (void)inExclusiveTransaction:(__attribute__((noescape)) void (^)(ACFMDatabase *db, BOOL *rollback))block {
    [self beginTransaction:FMDBTransactionExclusive withBlock:block];
}

- (void)inImmediateTransaction:(__attribute__((noescape)) void (^)(ACFMDatabase *db, BOOL *rollback))block {
    [self beginTransaction:FMDBTransactionImmediate withBlock:block];
}

- (NSError*)inSavePoint:(__attribute__((noescape)) void (^)(ACFMDatabase *db, BOOL *rollback))block {
