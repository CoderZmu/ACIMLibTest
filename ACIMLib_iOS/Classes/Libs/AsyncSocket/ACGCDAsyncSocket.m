...
#ifndef ACGCDAsyncSocketLoggingEnabled
#define ACGCDAsyncSocketLoggingEnabled 0
#endif

#if ACGCDAsyncSocketLoggingEnabled
...
#define LogAsync   YES
#define LogContext ACGCDAsyncSocketLoggingContext
...
#ifndef ACGCDAsyncSocketLogLevel
#define ACGCDAsyncSocketLogLevel LOG_LEVEL_VERBOSE
#endif

// Log levels : off, error, warn, info, verbose
static const int logLevel = ACGCDAsyncSocketLogLevel;
...
NSString *const ACGCDAsyncSocketException = @"ACGCDAsyncSocketException";
NSString *const ACGCDAsyncSocketErrorDomain = @"ACGCDAsyncSocketErrorDomain";

NSString *const ACGCDAsyncSocketQueueName = @"ACGCDAsyncSocket";
NSString *const ACGCDAsyncSocketThreadName = @"ACGCDAsyncSocket-CFStream";

NSString *const ACGCDAsyncSocketManuallyEvaluateTrust = @"ACGCDAsyncSocketManuallyEvaluateTrust";
#if TARGET_OS_IPHONE
NSString *const ACGCDAsyncSocketUseCFStreamForTLS = @"ACGCDAsyncSocketUseCFStreamForTLS";
#endif
NSString *const ACGCDAsyncSocketSSLPeerID = @"ACGCDAsyncSocketSSLPeerID";
NSString *const ACGCDAsyncSocketSSLProtocolVersionMin = @"ACGCDAsyncSocketSSLProtocolVersionMin";
NSString *const ACGCDAsyncSocketSSLProtocolVersionMax = @"ACGCDAsyncSocketSSLProtocolVersionMax";
NSString *const ACGCDAsyncSocketSSLSessionOptionFalseStart = @"ACGCDAsyncSocketSSLSessionOptionFalseStart";
NSString *const ACGCDAsyncSocketSSLSessionOptionSendOneByteRecord = @"ACGCDAsyncSocketSSLSessionOptionSendOneByteRecord";
NSString *const ACGCDAsyncSocketSSLCipherSuites = @"ACGCDAsyncSocketSSLCipherSuites";
NSString *const ACGCDAsyncSocketSSLALPN = @"ACGCDAsyncSocketSSLALPN";
#if !TARGET_OS_IPHONE
NSString *const ACGCDAsyncSocketSSLDiffieHellmanParameters = @"ACGCDAsyncSocketSSLDiffieHellmanParameters";
#endif

enum ACGCDAsyncSocketFlags
{
...
@interface ACGCDAsyncSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
@implementation ACGCDAsyncSocketPreBuffer
...
**/
@interface ACAsyncReadPacket : NSObject
{
  @public
...
- (NSUInteger)readLengthForTermWithPreBuffer:(ACGCDAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;

- (NSInteger)searchForTermAfterPreBuffering:(ssize_t)numBytes;

@end

@implementation ACAsyncReadPacket
...
**/
@interface ACAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation ACAsyncWritePacket
...
@implementation ACGCDAsyncSocket
{
	uint32_t flags;
	uint16_t config;
	
	__weak id<ACGCDAsyncSocketDelegate> delegate;
	...
	ACAsyncReadPacket *currentRead;
	ACAsyncWritePacket *currentWrite;
	
	unsigned long socketFDBytesAvailable;
	
	ACGCDAsyncSocketPreBuffer *preBuffer;
	...
	ACGCDAsyncSocketPreBuffer *sslPreBuffer;
...
- (instancetype)initWithDelegate:(id<ACGCDAsyncSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq
{
	return [self initWithDelegate:aDelegate delegateQueue:dq socketQueue:NULL];
}

- (instancetype)initWithDelegate:(id<ACGCDAsyncSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		delegate = aDelegate;
		...
		{
			socketQueue = dispatch_queue_create([ACGCDAsyncSocketQueueName UTF8String], NULL);
		}
		...
		preBuffer = [[ACGCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<ACGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq error:(NSError**)error {
  return [self socketFromConnectedSocketFD:socketFD delegate:aDelegate delegateQueue:dq socketQueue:NULL error:error];
}

+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<ACGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError* __autoreleasing *)error
{
  __block BOOL errorOccured = NO;
  
  ACGCDAsyncSocket *socket = [[[self class] alloc] initWithDelegate:aDelegate delegateQueue:dq socketQueue:sq];
  ...
  dispatch_sync(socket->socketQueue, ^{ @autoreleasepool {
    ...
    if (retVal)
    {
      NSString *errMsg = NSLocalizedStringWithDefaultValue(@"ACGCDAsyncSocketOtherError",
                                                           @"ACGCDAsyncSocket", [NSBundle mainBundle],
      ...
      errorOccured = YES;
      if (error)
        *error = [NSError errorWithDomain:ACGCDAsyncSocketErrorDomain code:ACGCDAsyncSocketOtherError userInfo:userInfo];
      return;
    }
...
- (void)setDelegate:(id<ACGCDAsyncSocketDelegate>)newDelegate
{
	[self setDelegate:newDelegate synchronously:NO];
}

- (void)synchronouslySetDelegate:(id<ACGCDAsyncSocketDelegate>)newDelegate
{
...
- (void)getDelegate:(id<ACGCDAsyncSocketDelegate> *)delegatePtr delegateQueue:(dispatch_queue_t *)delegateQueuePtr
{
	if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
...
- (void)setDelegate:(id<ACGCDAsyncSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
	[self setDelegate:newDelegate delegateQueue:newDelegateQueue synchronously:NO];
}

- (void)synchronouslySetDelegate:(id<ACGCDAsyncSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		__strong id<ACGCDAsyncSocketDelegate> theDelegate = delegate;
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			// Create ACGCDAsyncSocket instance for accepted socket
			
			ACGCDAsyncSocket *acceptedSocket = [[[self class] alloc] initWithDelegate:theDelegate
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:ACGCDAsyncSocketErrorDomain code:ACGCDAsyncSocketBadConfigError userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:ACGCDAsyncSocketErrorDomain code:ACGCDAsyncSocketBadParamError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"ACGCDAsyncSocketConnectTimeoutError",
	                                                     @"ACGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Attempt to connect to host timed out", nil);
	
	NSDictionary *userInfo = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:ACGCDAsyncSocketErrorDomain code:ACGCDAsyncSocketConnectTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"ACGCDAsyncSocketReadMaxedOutError",
														 @"ACGCDAsyncSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);
	
	NSDictionary *info = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:ACGCDAsyncSocketErrorDomain code:ACGCDAsyncSocketReadMaxedOutError userInfo:info];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"ACGCDAsyncSocketReadTimeoutError",
	                                                     @"ACGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Read operation timed out", nil);
	
	NSDictionary *userInfo = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:ACGCDAsyncSocketErrorDomain code:ACGCDAsyncSocketReadTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"ACGCDAsyncSocketWriteTimeoutError",
	                                                     @"ACGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Write operation timed out", nil);
	
	NSDictionary *userInfo = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:ACGCDAsyncSocketErrorDomain code:ACGCDAsyncSocketWriteTimeoutError userInfo:userInfo];
}

- (NSError *)connectionClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"ACGCDAsyncSocketClosedError",
	                                                     @"ACGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Socket closed by remote peer", nil);
	
	NSDictionary *userInfo = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:ACGCDAsyncSocketErrorDomain code:ACGCDAsyncSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:ACGCDAsyncSocketErrorDomain code:ACGCDAsyncSocketOtherError userInfo:userInfo];
}
...
- (BOOL)usingCFStreamForTLS
{
	...
	{
		// The startTLS method was given the ACGCDAsyncSocketUseCFStreamForTLS flag.
                        ...
                        tag:(long)tag
{
	...
	ACAsyncReadPacket *packet = [[ACAsyncReadPacket alloc] initWithData:buffer
	                                                          startOffset:offset
	                                                            maxLength:length
	                                                              timeout:timeout
	                                                           readLength:0
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
        ...
        if (!self->currentRead || ![self->currentRead isKindOfClass:[ACAsyncReadPacket class]])
		{
...
- (void)doReadData
{
	...
	if ([self usingCFStreamForTLS])
	{
		...
		// Requested CFStream, rather than SecureTransport, for TLS (via ACGCDAsyncSocketUseCFStreamForTLS)
...
- (void)completeCurrentRead
{
	...
	{
		ACAsyncReadPacket *theRead = currentRead; // Ensure currentRead retained since result may not own buffer
...
- (void)doReadTimeout
{
	...
	{
		ACAsyncReadPacket *theRead = currentRead;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	ACAsyncWritePacket *packet = [[ACAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
        ...
        if (!self->currentWrite || ![self->currentWrite isKindOfClass:[ACAsyncWritePacket class]])
		{
...
- (void)doWriteTimeout
{
	...
	{
		ACAsyncWritePacket *theWrite = currentWrite;
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		#if TARGET_OS_IPHONE
		{
			...
			NSNumber *value = [tlsSettings objectForKey:ACGCDAsyncSocketUseCFStreamForTLS];
...
{
	ACGCDAsyncSocket *asyncSocket = (__bridge ACGCDAsyncSocket *)connection;
...
- (void)ssl_startTLS
{
	...
	NSNumber *shouldManuallyEvaluateTrust = [tlsSettings objectForKey:ACGCDAsyncSocketManuallyEvaluateTrust];
	...
	//  3. ACGCDAsyncSocketSSLPeerID
	//  4. ACGCDAsyncSocketSSLProtocolVersionMin
	//  5. ACGCDAsyncSocketSSLProtocolVersionMax
	//  6. ACGCDAsyncSocketSSLSessionOptionFalseStart
	//  7. ACGCDAsyncSocketSSLSessionOptionSendOneByteRecord
	//  8. ACGCDAsyncSocketSSLCipherSuites
	//  9. ACGCDAsyncSocketSSLDiffieHellmanParameters (Mac)
    // 10. ACGCDAsyncSocketSSLALPN
	//
	...
	value = [tlsSettings objectForKey:ACGCDAsyncSocketSSLPeerID];
	...
	{
		NSAssert(NO, @"Invalid value for ACGCDAsyncSocketSSLPeerID. Value must be of type NSData."
		             @" (You can convert strings to data using a method like"
		             @" [string dataUsingEncoding:NSUTF8StringEncoding])");
		
		[self closeWithError:[self otherError:@"Invalid value for ACGCDAsyncSocketSSLPeerID."]];
		return;
	}
	
	// 4. ACGCDAsyncSocketSSLProtocolVersionMin
	
	value = [tlsSettings objectForKey:ACGCDAsyncSocketSSLProtocolVersionMin];
	...
	{
		NSAssert(NO, @"Invalid value for ACGCDAsyncSocketSSLProtocolVersionMin. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for ACGCDAsyncSocketSSLProtocolVersionMin."]];
		return;
	}
	
	// 5. ACGCDAsyncSocketSSLProtocolVersionMax
	
	value = [tlsSettings objectForKey:ACGCDAsyncSocketSSLProtocolVersionMax];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for ACGCDAsyncSocketSSLProtocolVersionMax. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for ACGCDAsyncSocketSSLProtocolVersionMax."]];
		return;
	}
	
	// 6. ACGCDAsyncSocketSSLSessionOptionFalseStart
	
	value = [tlsSettings objectForKey:ACGCDAsyncSocketSSLSessionOptionFalseStart];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for ACGCDAsyncSocketSSLSessionOptionFalseStart. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for ACGCDAsyncSocketSSLSessionOptionFalseStart."]];
		return;
	}
	
	// 7. ACGCDAsyncSocketSSLSessionOptionSendOneByteRecord
	
	value = [tlsSettings objectForKey:ACGCDAsyncSocketSSLSessionOptionSendOneByteRecord];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for ACGCDAsyncSocketSSLSessionOptionSendOneByteRecord."
		             @" Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for ACGCDAsyncSocketSSLSessionOptionSendOneByteRecord."]];
		return;
	}
	
	// 8. ACGCDAsyncSocketSSLCipherSuites
	
	value = [tlsSettings objectForKey:ACGCDAsyncSocketSSLCipherSuites];
	if ([value isKindOfClass:[NSArray class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for ACGCDAsyncSocketSSLCipherSuites. Value must be of type NSArray.");
		
		[self closeWithError:[self otherError:@"Invalid value for ACGCDAsyncSocketSSLCipherSuites."]];
		return;
	}
	
	// 9. ACGCDAsyncSocketSSLDiffieHellmanParameters
	
	#if !TARGET_OS_IPHONE
	value = [tlsSettings objectForKey:ACGCDAsyncSocketSSLDiffieHellmanParameters];
	if ([value isKindOfClass:[NSData class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for ACGCDAsyncSocketSSLDiffieHellmanParameters. Value must be of type NSData.");
		
		[self closeWithError:[self otherError:@"Invalid value for ACGCDAsyncSocketSSLDiffieHellmanParameters."]];
		return;
	}
	#endif

    // 10. kCFStreamSSLCertificates
    value = [tlsSettings objectForKey:ACGCDAsyncSocketSSLALPN];
    if ([value isKindOfClass:[NSArray class]])
    {
        ...
        {
            NSAssert(NO, @"Security option unavailable - ACGCDAsyncSocketSSLALPN"
                     @" - iOS 11.0, macOS 10.13 required");
            [self closeWithError:[self otherError:@"Security option unavailable - ACGCDAsyncSocketSSLALPN"]];
        }
    }
    else if (value)
    {
        NSAssert(NO, @"Invalid value for ACGCDAsyncSocketSSLALPN. Value must be of type NSArray.");
        
        [self closeWithError:[self otherError:@"Invalid value for ACGCDAsyncSocketSSLALPN."]];
        return;
    }
	...
	if (value)
	{
		NSAssert(NO, @"Security option unavailable - kCFStreamSSLLevel"
		             @" - You must use ACGCDAsyncSocketSSLProtocolVersionMin & ACGCDAsyncSocketSSLProtocolVersionMax");
	...
	sslPreBuffer = [[ACGCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
- (void)ssl_continueSSLHandshake
{
	...
	else if (status == errSSLPeerAuthCompleted)
	{
		...
		else
		{
			...
			NSString *msg = @"ACGCDAsyncSocketManuallyEvaluateTrust specified in tlsSettings,"
...
+ (void)startCFStreamThreadIfNeeded
{
	...
	dispatch_once(&predicate, ^{
		...
		cfstreamThreadSetupQueue = dispatch_queue_create("ACGCDAsyncSocket-CFStreamThreadSetup", DISPATCH_QUEUE_SERIAL);
	});
...
{
	[[NSThread currentThread] setName:ACGCDAsyncSocketThreadName];
...
+ (void)scheduleCFStreams:(ACGCDAsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(ACGCDAsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	ACGCDAsyncSocket *asyncSocket = (__bridge ACGCDAsyncSocket *)pInfo;
